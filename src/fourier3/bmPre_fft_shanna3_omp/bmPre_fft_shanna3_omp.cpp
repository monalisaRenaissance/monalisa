...
void bmPre_fft_shanna3_omp(	float* in_complex_ptr_0_shared, 
...
#pragma omp parallel shared(in_complex_ptr_0_shared, in_real_ptr_0_shared, in_imag_ptr_0_shared, Nx_u_shared, Ny_u_shared, Nz_u_shared, nCh_shared, KFC_real_ptr_0_shared, KFC_imag_ptr_0_shared)
	{
...
#pragma omp for
		for (int ind_ch = 0; ind_ch < nCh; ind_ch++)
		{
			...
			KFC_real_ptr_run = KFC_real_ptr_0 + start_incr + ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			KFC_imag_ptr_run = KFC_imag_ptr_0 + start_incr + ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			...
			KFC_real_ptr_run	-= Nx_u*Ny_u*Nz_u;
			KFC_imag_ptr_run	-= Nx_u*Ny_u*Nz_u;
			cz_max = Nz_u - cz_max;
			for (cz = 0; cz < cz_max; cz++)
			{
				cy_max = Ny_u - Ny_half;
