...
void pre_fft3(float* in_complex_ptr_0, float* in_real_ptr_0, float* in_imag_ptr_0, int Nx_u, int Ny_u, int Nz_u, int nCh);
void post_fft3(float* out_complex_ptr_0, float* out_real_ptr_0, float* out_imag_ptr_0, int Nx_u, int Ny_u, int Nz_u, int nCh, float F);

void bmDFT3_FFTW(float* out_real_ptr_0, float* out_imag_ptr_0, float* in_real_ptr_0, float* in_imag_ptr_0, int Nx_u, int Ny_u, int Nz_u, float dKx_u, float dKy_u, float dKz_u, int nCh)
{
	...
	float F = ((float)Nx_u)*((float)Ny_u)*((float)Nz_u)*dKx_u*dKy_u*dKz_u;
	...
	// n and m inverted; row versus column major order, and last dim has fastest varying index. 
	// but I don't see any logic in this order. 
	p = fftwf_plan_dft_3d(Nz_u, Ny_u, Nx_u, in_complex_ptr_0, out_complex_ptr_0, FFTW_FORWARD, FFTW_ESTIMATE); 


	// we copy the data in complex_interleaved order and do ifft shift at the same time
	pre_fft3((float*)in_complex_ptr_0, in_real_ptr_0, in_imag_ptr_0, Nx_u, Ny_u, Nz_u, nCh);
	...
	post_fft3((float*)out_complex_ptr_0, out_real_ptr_0, out_imag_ptr_0, Nx_u, Ny_u, Nz_u, nCh, F);
