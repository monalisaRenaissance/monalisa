...
#include <cmath>
#include <omp.h>
#include "bmPost_fft_nakatsha3_omp.h"

void bmPost_fft_nakatsha3_omp(	float* out_complex_ptr_0_shared,
...
#pragma omp parallel shared(out_complex_ptr_0_shared, \
							...
							nCh_shared)
	{
		...
		int cy_max;
		int cy;
		int y_half = (int)ceil(Ny_u_double / 2);

		int Nz_u = Nz_u_shared;
		double Nz_u_double = (double)Nz_u;
		int cz_max;
		int cz;
		int z_half = (int)ceil(Nz_u_double / 2);

		long long start_incr = ((long long)2) * (  ((long long)x_half) + ((long long)Nx_u) * ((long long)y_half) + ((long long)Nx_u) * ((long long)Ny_u) * ((long long)z_half)  );

		float F = 1.0f / (  ((float)Nx_u)*((float)Ny_u)*((float)Nz_u)  );
...
#pragma omp for
		...
		{
			inter_real_ptr_run = inter_real_ptr_0 + ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			inter_imag_ptr_run = inter_imag_ptr_0 + ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			KFC_conj_real_ptr_run = KFC_conj_real_ptr_0 + ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			KFC_conj_imag_ptr_run = KFC_conj_imag_ptr_0 + ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			out_complexR_ptr_run = out_complexR_ptr_0 + start_incr + ((long long)2)*((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			...
			for (cz = 0; cz < cz_max; cz++)
			{
				...
				out_complexR_ptr_run -= 2 * Nx_u * Ny_u;
				out_complexI_ptr_run -= 2 * Nx_u * Ny_u;
				cy_max = Ny_u - cy_max;
				for (cy = 0; cy < cy_max; cy++)
				{
					cx_max = Nx_u - x_half;
			...
			out_complexR_ptr_run -= 2 * Nx_u * Ny_u * Nz_u;
			out_complexI_ptr_run -= 2 * Nx_u * Ny_u * Nz_u;
			cz_max = Nz_u - cz_max;
			for (cz = 0; cz < cz_max; cz++)
			{
				cy_max = Ny_u - y_half;
	...
	{
		int zero_ind;
		int zero_ind_max = (Nx_u_shared * Ny_u_shared * Nz_u_shared);
	...
	omp_set_num_threads(omp_get_max_threads());
#pragma omp parallel shared(out_real_ptr_0_shared, \
							out_imag_ptr_0_shared, \
							inter_real_ptr_0_shared, \
							inter_imag_ptr_0_shared, \
							Nx_u_shared, \
							...
							nCh_shared)
	{
		...
		int M = (Nx_u_shared * Ny_u_shared * Nz_u_shared);
		int N = nCh_shared;
		int j;
