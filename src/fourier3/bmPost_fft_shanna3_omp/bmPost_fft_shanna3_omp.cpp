...
#include <cmath>
#include <omp.h>
#include "bmPost_fft_shanna3_omp.h"
// #include "mex.h"

void bmPost_fft_shanna3_omp(float* out_complex_ptr_0_shared, 
...
#pragma omp parallel shared(out_complex_ptr_0_shared, out_real_ptr_0_shared, out_imag_ptr_0_shared, Nx_u_shared, Ny_u_shared, Nz_u_shared, nCh_shared)
	{
		...
		long long start_incr = ((long long)2) * (   ((long long)Nx_half) + ((long long)Nx_u) * ((long long)Ny_half) + ((long long)Nx_u) * ((long long)Ny_u) * ((long long)Nz_half)   );
...
#pragma omp for
		...
		{
			out_complex_ptr_run = out_complex_ptr_0 + start_incr + ((long long)2)*((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			out_real_ptr_run	= out_real_ptr_0	+ ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
			out_imag_ptr_run	= out_imag_ptr_0	+ ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u)*((long long)Nz_u);
