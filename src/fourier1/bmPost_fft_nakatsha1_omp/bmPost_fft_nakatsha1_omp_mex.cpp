...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	float* z_complex_ptr_0   = new float[2 * nCh * Nx_u];
	float* z_complex_ptr_run = z_complex_ptr_0; 
	float* inter_complex_ptr_0 = new float[2 * nCh * Nx_u];
	float* inter_real_ptr_0 = inter_complex_ptr_0;
	float* inter_imag_ptr_0 = inter_complex_ptr_0 + nCh * Nx_u;
	

	for (int i = 0; i < nCh*Nx_u; i++)
	{
	...
	// function call
	bmPost_fft_nakatsha1_omp(	z_complex_ptr_0,
								out_real_ptr_0,
								out_imag_ptr_0,
								inter_real_ptr_0,
								inter_imag_ptr_0,
