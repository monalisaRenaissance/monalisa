...
for i = 1:nCell
    ...
    if ~isempty(maxPixDisplacement)
        ...
        elseif frDim == 3
            v_reg = bmImDeformField_hardThresholding3(v_reg, n_u, maxPixDisplacement);
        end
    end
    imDeformField{i}    = bmPermuteToPoint(v_reg, n_u); 
...
temp_frDim   = size(temp_size(:), 1) - 1;
temp_N_u     = temp_size(1, 1:temp_frDim);
