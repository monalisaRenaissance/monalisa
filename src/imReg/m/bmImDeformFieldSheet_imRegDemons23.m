...
for i = 1:nCell_1
    for j = 1:nCell_2
        ...
        im_first    = single(private_smooth(x{1, 1},          nSmooth));
        ...
        if strcmp(sheet_type,        'curr_to_prev')
            [v_reg_1, tmp_im_1]  = imregdemons(im_curr, im_prev_1, nIter);
            tmp_im_1             = tmp_im_1*max_curr/maxImVal + min_curr;
            
            [v_reg_2, tmp_im_2]  = imregdemons(im_curr, im_prev_2, nIter);
            tmp_im_2             = tmp_im_2*max_curr/maxImVal + min_curr;
            
        elseif strcmp(sheet_type,    'curr_to_next')
            [v_reg_1, tmp_im_1]  = imregdemons(im_curr, im_next_1, nIter);
            tmp_im_1             = tmp_im_1*max_curr/maxImVal + min_curr;
            
            [v_reg_2, tmp_im_2]  = imregdemons(im_curr, im_next_2, nIter);
            tmp_im_2             = tmp_im_2*max_curr/maxImVal + min_curr;
            
        elseif strcmp(sheet_type,    'prev_to_curr')
            [v_reg_1, tmp_im_1]  = imregdemons(im_prev_1, im_curr, nIter);
            tmp_im_1             = tmp_im_1*max_prev_1/maxImVal + min_prev_1;
            
            [v_reg_2, tmp_im_2]  = imregdemons(im_prev_2, im_curr, nIter);
            tmp_im_2             = tmp_im_2*max_prev_2/maxImVal + min_prev_2;
            
        elseif strcmp(sheet_type,    'next_to_curr')
            [v_reg_1, tmp_im_1]  = imregdemons(im_next_1, im_curr, nIter);
            tmp_im_1             = tmp_im_1*max_next_1/maxImVal + min_next_1;
            
            [v_reg_2, tmp_im_2]  = imregdemons(im_next_2, im_curr, nIter);
        ...
        elseif strcmp(chain_type,    'curr_to_first')
            ...
            if (i == 1) && (j == 1)
                v_reg_1 = [];
            elseif i > 1 || j > 1
                [v_reg_1, tmp_im_1] = imregdemons(im_curr, im_first, nIter);
                tmp_im_1            = tmp_im_1*max_curr/maxImVal + min_curr;
            end
        ...
        elseif strcmp(chain_type,    'first_to_curr')
            ...
            if (i == 1) && (j == 1)
                v_reg_1 = [];
            elseif i > 1 || j > 1
                [v_reg_1, tmp_im_1] = imregdemons(im_first, im_curr, nIter);
                tmp_im_1            = tmp_im_1*max_first/maxImVal + min_first;
            end
        ...
        if ~isempty(myMask) && ~isempty(v_reg_1)
            v_reg_1 = v_reg_1.*myMask;
        end
        
        if ~isempty(myMask) && ~isempty(v_reg_2)
            v_reg_2 = v_reg_2.*myMask;
        end
        
        if ~isempty(maxPixDisplacement) && ~isempty(v_reg_1)
            v_reg_1 = bmImDeformField_hardThresholding2(v_reg_1, n_u, maxPixDisplacement);
        end
        
        if ~isempty(maxPixDisplacement) && ~isempty(v_reg_2)
