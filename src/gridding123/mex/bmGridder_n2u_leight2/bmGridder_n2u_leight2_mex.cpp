...
// very early stage of developpment of the
// reconstruction code.
...
void bmGridder_n2u_leight2_mex(	float* aReal_ptr0, 
								float* aImag_ptr0, 
								float* bReal_ptr0, 
								float* bImag_ptr0, 
								float* tx_ptr0, 
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	bmGridder_n2u_leight2_mex(	aReal_ptr0, 
								aImag_ptr0, 
								bReal_ptr0, 
								bImag_ptr0, 
								tx_ptr0, 
		...
		Dn			= *Dn_run++; 
			...
			for (ind_y = 0; ind_y < nWin; ind_y++)
			{
				...
				for (ind_x = 0; ind_x < nWin; ind_x++)
				{
					...
					nx_64			= (long long)nx; 
					ny_64			= (long long)ny;
					normalize_run	= normalize_ptr0	+ (nx_64		+ ny_64*Nx_64); 
					bReal_ptr1		= bReal_ptr0		+ (nx_64*nCh_64 + ny_64*Nx_nCh_64);
					bReal_run		= bReal_ptr1; 
					bImag_ptr1		= bImag_ptr0		+ (nx_64*nCh_64 + ny_64*Nx_nCh_64);
