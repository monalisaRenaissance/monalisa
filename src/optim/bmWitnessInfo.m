...
classdef bmWitnessInfo < handle
    ...
    methods
        ...
        function watch(obj, curr_ind, x, n_u, initial_or_loop_or_final, varargin)
            
            myName = obj.witness_name;
            
            % If we are in the loop, we print info accordingly, depending
            % if we are in the first itaration or not. 
            if strcmp(initial_or_loop_or_final, 'loop')
            ...
            % If witness_ind is not empty we apply the witness-mechanism. 
            if ~isempty(obj.witness_ind) 
                ...
                if ~strcmp(initial_or_loop_or_final, 'final')
                    if imDim == 1
                        if iscell(x)
                            temp_im = (x{1});
                        else
                ...
                % The final step is like any other excpet we print the
                % final message. 
                if strcmp(initial_or_loop_or_final, 'final')
