...
void bmPost_fft_nakatsha2_omp(	float* out_complex_ptr_0_shared,
...
#pragma omp parallel shared(out_complex_ptr_0_shared, \
							...
							nCh_shared)
	{
		...
		long long start_incr = ((long long)2) * (  ((long long)x_half) + ((long long)Nx_u) * ((long long)y_half)  );

		float F = 1.0f / (  ((float)Nx_u)*((float)Ny_u)  );
...
#pragma omp for
		...
		{
			inter_real_ptr_run		= inter_real_ptr_0		+ ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u);
			inter_imag_ptr_run		= inter_imag_ptr_0		+ ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u);
			KFC_conj_real_ptr_run	= KFC_conj_real_ptr_0	+ ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u);
			KFC_conj_imag_ptr_run	= KFC_conj_imag_ptr_0	+ ((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u);
			out_complexR_ptr_run	= out_complexR_ptr_0	+ start_incr + ((long long)2)*((long long)ind_ch)*((long long)Nx_u)*((long long)Ny_u);
	...
	{
		int zero_ind;
		int zero_ind_max = (Nx_u_shared * Ny_u_shared);
	...
	omp_set_num_threads(omp_get_max_threads());
#pragma omp parallel shared(out_real_ptr_0_shared, out_imag_ptr_0_shared, inter_real_ptr_0_shared, inter_imag_ptr_0_shared, Nx_u_shared, Ny_u_shared, nCh_shared)
	{

		// mexPrintf("This is thread number %d .\n", omp_get_thread_num()); 

		int M = (Nx_u_shared * Ny_u_shared);
		int N = nCh_shared;
		int j;
