...
% initialize z's
if isempty(z1)
    z1   = private_F1(x, Tu1, n_u, nFr, K_bump); 
end
if isempty(z2)
    z2   = private_F2(x, Tu2, n_u, nFr, K_bump); 
end
% initialize u's
if isempty(u1)
...
while convCond.check()
    ...
    res_z1_next             = bmMinus(  bmMinus(z1, u1),  private_F1(x, Tu1, n_u, nFr, K_bump)      );
    res_z2_next             = bmMinus(  bmMinus(z2, u2),  private_F2(x, Tu2, n_u, nFr, K_bump)      );
    
    dagM_res_y_next         = private_dagM(res_y_next, Gut, HX, HY, n_u, nFr, KFC_conj);
    
    dagF1_res_z1_next       = bmMult(rho, private_dagF1(res_z1_next, Tu1t, HX, HZ1, n_u, nFr, K_bump));
    dagF2_res_z2_next       = bmMult(rho, private_dagF2(res_z2_next, Tu2t, HX, HZ2, n_u, nFr, K_bump));
    ...
    % updating_z_and_u ---------------------------------------------------- 
    F1x_plus_u1              = bmPlus(u1, private_F1(x, Tu1, n_u, nFr, K_bump)); 
    F2x_plus_u2              = bmPlus(u2, private_F2(x, Tu2, n_u, nFr, K_bump)); 
