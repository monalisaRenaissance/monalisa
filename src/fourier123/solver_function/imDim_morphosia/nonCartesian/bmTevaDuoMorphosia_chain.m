...
% function_label
function_label = 'bmTevaDuoMorphosia_chain'; 
...
HZ1                     = single(HX); % We identify the sparse representation space Z with image-space X since it is natural to consider the difference between two images as an image. 
HZ2                     = single(HX); % We identify the sparse representation space Z with image-space X since it is natural to consider the difference between two images as an image. 
...
% initila_monitoring ------------------------------------------------------
[dafi, regul] = private_dafi_regul(x, y, Gu, Tu1, Tu2, HY, deltaR, frSize, nFr, KFC, K_bump);
...
for c = 1:nIter
    ...
    z1                       = bmProx_oneNorm(F1x_plus_u1, delta*deltaR/rho./HZ1);
    z2                       = bmProx_oneNorm(F2x_plus_u2, delta*deltaR/rho./HZ2);
...
function [dafi, regul]    = private_dafi_regul(x, y, Gu, Tu1, Tu2, HY, deltaR, frSize, nFr, KFC, K_bump) 
    dafi   = 0;
    regul  = 0; 
    for i = 1:nFr
        ...
        regul       = regul + deltaR*sum(abs(  real(temp_res(:))  )) + deltaR*sum(abs(  imag(temp_res(:))  ));      
