...
private_init_witnessInfo(witnessInfo, nIter, 'bmTevaMorphosia_all_to_first', frSize, N_u, dK_u, delta, rho, nCGD, ve_max); 
    ...
    for j = 1:nCGD
        ...
        for i = 1:nFr
            ...
            if i == 1
                dagF_res_z_next = 0;
                for k = 2:nFr
                    dagF_res_z_next = dagF_res_z_next - (1/HX)*rho_HZ*res_z_next{k};
                end
            else
                dagF_res_z_next = (1/HX)*bmImDeformT(Tut{i}, rho_HZ*res_z_next{i}, frSize, K_bump); % negative_gradient
            end
...
function private_init_witnessInfo(witnessInfo, nIter, arg_name, frSize, N_u, dK_u, delta, rho, nCGD, ve_max)
