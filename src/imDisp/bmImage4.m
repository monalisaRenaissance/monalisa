...
function varargout = bmImage4(argImagesTable, varargin)
...
function myPoint = hard_coord(myPoint)
    ...
    function show_imVal_in_title(soft_point)
        ...
        mySize = size(myImagesTable); 

        soft_point(1, 1) = min(mySize(1, 1), max(1, soft_point(1, 1)));
        soft_point(2, 1) = min(mySize(1, 2), max(1, soft_point(2, 1)));
        soft_point(3, 1) = min(mySize(1, 3), max(1, soft_point(3, 1)));
    ...
    function set_point
        ...
        temp_ind = myParam.curImNum_4; 
        myParam.point_list{temp_ind}  = cat(2, myParam.point_list{temp_ind}, hard_point);
        point_list{temp_ind}          = [point_list{temp_ind}, soft_point];
    ...
    function delete_point
        if ~isempty(myParam.point_list) && ~isempty(point_list)
            temp_ind = myParam.curImNum_4;
            if ~isempty(myParam.point_list{temp_ind}) && ~isempty(point_list{temp_ind})
                myParam.point_list{temp_ind}(:, end)  = [];
                point_list{temp_ind}(:, end)          = [];
            end
        end
        refresh_image;
    end
    ...
    function rotate_image()
        ...
        for i = 1:myParam.numOfImages_4
            temp_imagesTable = interpn( temp_X, temp_Y, temp_Z, ...
                                        myImagesTable(:, :, :, i), ...
                                        new_X, new_Y, new_Z);
            myImagesTable(:, :, :, i) = reshape(temp_imagesTable, myParam.imSize(1, 1:3));
        end
        myImagesTable(isnan(myImagesTable)) = 0;
        
        % update_point_list
        if ~isempty(point_list)
            if ~isempty(point_list{myParam.curImNum_4})
                
                
                temp_point = point_list{myParam.curImNum_4}; 
                
                temp_point = temp_point - repmat(  myShift, [1, size(temp_point, 2 )]  );
                temp_point = R\temp_point;
                temp_point = temp_point + repmat(  myShift, [1, size(temp_point, 2 )]  );
                
                point_list{myParam.curImNum_4} = temp_point; 
    ...
    function permute_image()
        ...
        elseif isequal(myParam.permutation, [3, 1, 2])
            ...
            myImagesTable = permute(myImagesTable, [myParam.permutation, 4]);
            
            myParam.curImNum = max([myParam.curImNum, 1]);
            myParam.curImNum = min([myParam.curImNum, myParam.numOfImages]);
            
            if length(myParam.point_list) > 0
                if ~isempty(myParam.point_list{1})
    ...
    function transpose_image()
        if myParam.transpose_flag
            ...
            myImagesTable = permute(myImagesTable, [2, 1, 3, 4]);
    ...
    function refresh_image()
        ...
        % display image ---------------------------------------------------
        figure(myFigure);
        imagesc(    myImagesTable(:, : , myParam.curImNum, myParam.curImNum_4), ...
                    myParam.colorLimits);
        ...
        if ~isempty(point_list)
            ...
            if ~isempty(point_list{myParam.curImNum_4})
                p = point_list{myParam.curImNum_4};
                for i = 1:size(p, 2)
    ...
    function set_inPlane_angle()
        ...
        temp_image                  = myImagesTable(:, : , myParam.curImNum, myParam.curImNum_4);
